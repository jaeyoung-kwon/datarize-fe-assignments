...
export const fetcher = {
  ...
  patch: async <TRequest extends JsonBody, TResponse>({ path, query, body }: FetcherOptions<TRequest>) =>
    request<TRequest, TResponse>({ path, query, body, method: 'PATCH' }),
  put: async <TRequest extends JsonBody, TResponse>({ path, body }: FetcherOptions<TRequest>) =>
    request<TRequest, TResponse>({ path, body, method: 'PUT' }),
  delete: async <TRequest extends JsonBody, TResponse>({ path, body }: FetcherOptions<TRequest>) =>
