...
import { Separator, Spinner } from '@/styles/styled'
...
const StatValue = styled.p<{ accent?: boolean }>`
  font-size: 1.5rem;
  font-weight: 700;
  color: ${({ accent }) => (accent ? theme.colors.accent : theme.colors.foreground)};
`
    ...
    <Overlay isOpen={open} onClick={handleOverlayClick}>
      <DialogContainer onClick={(e) => e.stopPropagation()}>
        ...
        <StatsGrid>
          ...
          <StatItem>
            <StatValue accent>{customer ? formatCurrency(customer.totalAmount) : '₩0'}</StatValue>
                    ...
                    <PurchaseGroup key={date}>
                      ...
                      {datePurchases?.map((purchase, index) => (
                        <PurchaseItem key={index}>
                          <Thumbnail>
                            <ThumbnailImage src={purchase.imgSrc} alt={purchase.product} />
                          </Thumbnail>
                          <ProductInfo>
